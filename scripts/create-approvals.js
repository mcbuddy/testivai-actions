 ...
 * @param {string} options.outputPath - Path to write the approvals.json file (default: .testivai/visual-regression/approvals.json)
...
if (require.main === module) {
  ...
  if (!author || !prNumber || !repoOwner || !repoName || !commitSha) {
    ...
    console.error('node create-approvals.js <author> <prNumber> <repoOwner> <repoName> <commitSha> [outputPath]');
    console.error('Or set environment variables: GITHUB_COMMENTER, PR_NUMBER, REPO_OWNER, REPO_NAME, COMMIT_SHA, APPROVALS_PATH');
    process.exit(1);
  }
